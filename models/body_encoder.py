...
from models.graph_layers import GraphResBlock, GraphLinear, spmm, batch_sparse_dense_matmul
...
class ds_us_fn(nn.Module):
    ...
    def forward(self, x):
        if x.ndimension() < 3:
            ...
            B, C, V = x.shape
            x = x.permute(2, 0, 1).reshape(V, B*C)
            x = torch.matmul(self.M, x).reshape(-1, B, C).permute(1, 2, 0)
        return x
...
class BodyEncoder(nn.Module):
    ...
    def forward(self, body_vertices, contact_features, generation_code):
        """Forward pass
        """
        batch_size, num_vertices = body_vertices.shape[:2]
        body_vertices = body_vertices.transpose(1, 2)
        feature = self.encoder(
            torch.cat([body_vertices,
                       contact_features.transpose(1, 2),
                       generation_code.unsqueeze(2).expand(-1, -1, num_vertices)], dim=1)
        )
        ...
        decoder_input = torch.cat((z.unsqueeze(2).expand(-1, -1, self.num_vertices),
                                   generation_code.unsqueeze(2).expand(-1, -1, self.num_vertices),
                                   self.ref_vertices.unsqueeze(0).expand(batch_size, -1, -1)), dim=1)

        pred = self.decoder(decoder_input).transpose(1, 2)
        x_rec = pred[:, :, :3]
        pred_f = pred[:, :, 3:]
        if self.args.residual:
            residual_f = self.residual_net(
                torch.cat([generation_code.unsqueeze(2).expand(-1, -1, self.mesh.num_vertices[self.args.init_downsample_level]),
    ...
    def sample(self, batch_size, generation_code):
