...
from models.dataset import NarratorDataset

from models.smplx_regressor import SMPLX_Regressor, SMPLX_Regressor_Joint, SMPLX_Regressor_Joint_Orient
from models.loss import *
from data.utils import *
from utils import *
from utils.chamfer_distance import chamfer_contact_loss, chamfer_dists
from utils.viz_util import render_composite_generation_multview, render_body_multview
...
class Litgeneration(pl.LightningModule):
    def __init__(self, args):
        ...
        if args.model =='generationVAE':
            self.model = generationVAE(args)
        else:
    ...
    def _common_step(self, batch, batch_idx, mode):
        ...
        # render reconstructed and sampled generations
        ...
        if (batch_idx % render_interval == 0) and (self.current_epoch >= self.args.render_epoch or self.args.debug):
            ...
            for idx in range(min(batch_size, render_num)):
                ...
                body_sample = skeleton_to_mesh(x_sample[idx].detach().cpu().numpy(), colors) \
                    ...
                    vertex_colors=colors)
                obj_meshes = []
                for obj_idx in batch['generation_obj_ids'][idx]:
                    if obj_idx != -1:
                ...
                base_name = mode + '_E{:03d}_It{:04d}_id{:d}_{}.png'.format(
                                                self.current_epoch, batch_idx, idx, batch['generation'][idx])
                ...
                export_file.parent.mkdir(exist_ok=True, parents=True)
                img_collage = render_composite_generation_multview(body_hat + coord_frame, obj_meshes,
                                                                    collage_mode='grid',
                ...
                img_collage = render_composite_generation_multview(body_sample + coord_frame, obj_meshes,
                                                          collage_mode='grid',
                                                                    use_material=False, smooth_body=False,
                                                          obj_points_coord=obj_points_coord[idx, :batch['num_atomics'][idx], :, :])
                img_collage.save(str(export_file))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model", type=str, default='generationVAE')
    ...
    parser.add_argument("--generation_bias", type=int, default=0)
    ...
    # dataset
    parser.add_argument("--used_generation", type=str, default='all')
    ...
    test_dataset = generationDataset(test_data if args.used_instance is None else train_data,
                                      num_points=args.num_obj_points, use_augment=False,
                                      include_motion=args.include_motion,
                                      used_generation=args.used_generation, split='test',
    ...
    train_dataset = generationDataset(train_data, num_points=args.num_obj_points, use_augment=args.use_augment,
    ...
    if args.resume_checkpoint is not None:
        ...
        model = Litgeneration.load_from_checkpoint(args.resume_checkpoint, args=args)
    else:
        print('start training from scratch')
        model = Litgeneration(args)

    # callback
    tb_logger = pl_loggers.TensorBoardLogger(str(results_folder / 'generation'), name=args.expr_name)
