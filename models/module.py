...
from models.graph_layers import GraphConvolution, GraphLinear
...
class GraphEncoder(nn.Module):
    """
    Get embedding for scene graph tokens
    """
	def __init__(self, input_dimension, output_dimension, adjmat, bias=True):
        super(GraphEncoder, self).__init__()
        self.use_nerf_embed = use_nerf_embed
		self.cov = GraphConvolution(input_dimension, output_dimension, adjmat, bias=True)
        self.linear = GraphLinear(input_dimension, output_dimension)

    def forward(self, scene_graphs):
        scene_graphs = self.linear(self.cov(scene_graphs))
        return scene_graphs

class TransformerVAE(nn.Module):
    """
    Conditional VAE network.
    """
